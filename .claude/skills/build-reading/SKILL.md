...
- **Submit for credit**: `ReadingKit.submitForCredit({readingId, title, tags, onDone})` sends the score to a private Google Sheet only the instructor can see, via a **Google Form's public response endpoint** (`docs.google.com/forms/d/e/<FORM_ID>/formResponse`) — not a GitHub repo (any repo collaborator can see every issue in it, defeating per-student privacy) and not a custom Apps Script Web App either (`assets/SubmissionsAppsScript.gs` is kept for reference but NOT used — the marcylabschool.org Google Workspace domain has an admin policy blocking anonymous access to Apps Script web apps, confirmed unfixable from an individual account). `SUBMISSIONS_FORM_ID` and the `SUBMISSIONS_FORM_ENTRIES` map near the top of `reading-kit.js` must be set to the real form/field IDs before this works — every reading shares the same constants. **`tags` must be passed on every call**, matching that reading's `reading.meta.json` tags array exactly, so skill-level performance is trackable in the Sheet. Because a public Form has no server-side code, there's no shared-secret gate and no server-enforced attempt cap — the 2-attempt limit is client-side (`localStorage`) only. Accepted trade-off for an internal, low-stakes formative-practice tool; state this plainly if asked, don't imply it's more locked-down than it is. The `DA-Reading_Submissions` GitHub repo is retired for grading — leave it alone, don't route new readings through it.
