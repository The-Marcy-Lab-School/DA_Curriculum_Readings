...
- **Submit for credit**: `ReadingKit.submitForCredit({readingId, title, tags, onDone})` opens a pre-filled GitHub Issue in the **student's own private submission repo** (created by each student from the `The-Marcy-Lab-School/DA-Reading_Submissions` template — that repo's own README has the student-facing setup steps), with the instructor added as the only other collaborator. Not a shared repo — that was tried first and any repo collaborator can see every other collaborator's issues, which defeats per-student privacy. Not a Google Form or Apps Script Web App either — both were tried and both hit the same marcylabschool.org Google Workspace admin policy blocking anonymous/external access, confirmed unfixable from an individual account across multiple approaches (`assets/SubmissionsAppsScript.gs` is kept for reference only, clearly marked unused). GitHub Issues never had that problem; the fix was scoping to one repo per student, not abandoning GitHub.
  ...
  - **`tags` must be passed on every call**, matching that reading's `reading.meta.json` tags array exactly, so skill-level performance is trackable later.
...
1. **Anti-duplication check + skill lookup.** Read the target lecture's deck notes; note what to avoid. In the same pass, find that lesson's entry in `skills.json` (regenerate it first via `python3 extract_skills.py` if the deck is new) and note its `objectives` — these are the deck author's own stated Learning Objectives, not something to invent. Decide which ones this reading actually preps for (usually all of them for a single-lesson reading).
...
4. **Author `index.html`** using `assets/brand-tokens.css` + `assets/reading-kit.css` + `assets/reading-kit.js`. Near the top (after the persona widget, before section 1), add a visible "Skills you'll practice in this reading" callout listing the matched `skills.json` objective text verbatim — it's already phrased safely (action-oriented, no lecture/module reference), don't reword it into something vaguer. Write a sidecar `reading.meta.json` (`{title, tags: [taxonomy.json ids], skills: [skills.json objective ids], audience, time_minutes}`) next to it. Pass both `tags` and `skills` into the `ReadingKit.submitForCredit({..., tags, skills})` call so instructor-side analytics can report at both the broad-category and the specific-objective level.
...
- `skills.json` — *specific* per-lesson Learning Objectives, generated by `extract_skills.py` from `module_decks/*/INSTRUCTOR_LECTURE_NOTES.md`. This is the source for `reading.meta.json`'s `skills` field and the in-reading "Skills you'll practice" callout — re-run `extract_skills.py` whenever a new deck lands, never hand-edit `skills.json` directly.
